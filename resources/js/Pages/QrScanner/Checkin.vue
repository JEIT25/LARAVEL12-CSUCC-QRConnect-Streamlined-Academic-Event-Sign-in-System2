<template>
    <div class="flex flex-col-reverse md:flex-row md:h-30">
        ...
        <div class="md:w-1/2 flex justify-center items-center  border-blue-500 p-4">
            ...
            <div class="w-full max-w-md">
                ...
                <div v-if="attendee_record" class="mt-4 bg-white p-4 rounded-lg shadow-md">
                    <div class="text-center capitalize">
                        ...
                        <p v-if="attendee_record.checkin"><strong>Check-in Time:</strong> {{
                            convertToLocalDateTime(attendee_record.checkin) }}</p>
                        <p v-else><strong>Check-in Time:</strong> {{ convertToLocalDateTime(checkin) }}</p>
                    </div>
                </div>
...
const checkin = ref(null); // In case of an existing attendee_record, this will be set with checkin datetime value of the existing attendee_record
const props = defineProps({
    event: Object
});

function convertToLocalDateTime(isoDate) { // Function to format the datetime value of checkin
...
onMounted(() => {
    const onScanSuccess = (qrCodeMessage) => {
        ...
        axios.post(`/events/${props.event.event_id}/qrscanner/checkin`, {
